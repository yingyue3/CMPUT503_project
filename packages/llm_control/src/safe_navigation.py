...
class SafeNavigationNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        # self.ARC_TARGET = np.pi*((self.wheelbase/2)+0.54) / 10
        self.ARC_TARGET = 0.3
    ...
    def preprocess_image(self, raw_image):
        ...
        resized_image = raw_image[150:239+100, 150:450]
        # new_width = 400
        # new_height = 300
    ...
    def detect_robot(self, imageFrame):
        # add your code here
        # potentially useful in question 2.1
        ...
        # Set white pixels to white (255)
        # lane_mask = np.zeros_like(blue_mask)
        # lane_mask[blue_mask > 0] = 225
        # rospy.loginfo("Yellow as lane")
    ...
    def move_straight(self,  speed=0.5, direction=1, distance=0.2, calibrate = 1.3):
        msg = WheelsCmdStamped()
        msg.vel_left = speed * direction * 1.3
        msg.vel_right = speed * direction
    ...
    def turn_right(self, speed=0.7, calibrate = 1):
        ...
        msg.vel_right = 0
        self.wheel_publisher.publish(msg)
        rospy.loginfo("Turn Right Complete.")
        pass
        
    def turn_left(self, speed=0.5, calibrate = 1):
        # add your code here
        msg = WheelsCmdStamped()
        ...
        while np.abs(self.start_dist - self.compute_distance_traveled(self._ticks_left)) < self.ARC_TARGET*0.1 and not rospy.is_shutdown():
            self.wheel_publisher.publish(msg)
            # rospy.sleep(0.1)
        ...
        msg.vel_right = 0
        self.wheel_publisher.publish(msg)
        rospy.loginfo("Turn Left Complete.")
        pass
        ...
        while not rospy.is_shutdown():
            ...
            if self.blue_lane:
                if not self.manipulate:
                    rospy.loginfo("Execute to aviod crashing")
                    self.stop()
                    rospy.sleep(3)
                    self.turn_left()
                    ...
                    rospy.loginfo("Yellow Lane Following")
                    if np.abs(self.start_dist - self.compute_distance_traveled(self._ticks_left)) < 0.3 and not rospy.is_shutdown():
                        self.black_detect_image = self.detect_robot(cv.blur(self.disorted_image, (5, 5)))
                ...
                rospy.loginfo("Normal Execution")
